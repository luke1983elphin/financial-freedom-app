(function bootFinancialFreedomApp() {
  ...
  function dashboardMissionHtml(result, readyState) {
    ...
    const nextTask = mission.tasks.find((task) => !task.completed) || mission.tasks[0];
    const completedTask = mission.tasks.find((task) => task.completed);
    const unfinishedTasks = mission.tasks.filter((task) => !task.completed);
    const previewTasks = [nextTask, completedTask, ...unfinishedTasks].filter(Boolean);
    const uniquePreview = Array.from(new Map(previewTasks.map((task) => [task.id, task])).values()).slice(0, 3);
    const visibleTasks = engagementMissionExpanded ? mission.tasks : uniquePreview;
    const moreCount = Math.max(0, mission.tasks.length - uniquePreview.length);
    return `
      <article class="dashboard-compact-card dashboard-mission-card">
        ...
        ${nextTask ? `
          <div class="dashboard-current-action">
            ...
            <button class="dashboard-task-button" type="button" data-engagement-action="weeklyplan">
              <strong>${escapeHtml(nextTask.title)}</strong>
              <span>Continue Weekly Plan</span>
            </button>
          </div>
        ` : ""}
