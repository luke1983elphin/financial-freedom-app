...
const baselineDir = path.resolve(currentDir, "..", "stage-e1-rental-property-ui-20260817");
...
function projectionSnapshot(context) {
  ...
  return {
    summary: {
      firstPersonFullRetirement: outcome.result.summary.firstPersonFullRetirement,
      householdFullRetirement: outcome.result.summary.householdFullRetirement,
      accessibleBalanceAtEndAge: outcome.result.summary.accessibleBalanceAtEndAge,
      superBalanceAtEndAge: outcome.result.summary.superBalanceAtEndAge,
      totalInvestableAssetsAtEndAge: outcome.result.summary.totalInvestableAssetsAtEndAge,
      totalNetWorthAtEndAge: outcome.result.summary.totalNetWorthAtEndAge,
      totalUnfundedSpending: outcome.result.summary.totalUnfundedSpending,
    },
...
writeFileSync(
  path.join(currentDir, "STAGE-F-NUMERICAL-COMPARISON.txt"),
  [
    "Stage F numerical comparison against Stage E1 tested copy",
    `Baseline: ${baselineDir}`,
    `Current: ${currentDir}`,
    "Result: PASS - representative semi-retirement projection snapshot is numerically identical.",
    "Fields compared: retirement milestones, employment income, tax, Medicare, MLS, lifestyle spending, required withdrawals, optional draws, surplus, accessible investments, super, property cashflow, debt, property equity and net worth.",
  ].join("\n"),
);
